...
if [[ "${DEFAULT_CONF}" =~ ^[eE][nN][aA][bB][lL][eE]$ ]]; then
	...
	cat ${Nginx_Conf_Dir}/nginx.conf > ${NGINX_INSTALL_DIR}/conf/nginx.conf
	...
	if [[ "${PHP_FPM}" =~ ^[eE][nN][aA][bB][lL][eE]$ ]]; then
		...
		sed -i "s/PHP_FPM_SERVER/${PHP_FPM_SERVER}/" ${NGINX_INSTALL_DIR}/conf/nginx.conf
		sed -i "s/PORT/${PHP_FPM_PORT}/" ${NGINX_INSTALL_DIR}/conf/nginx.conf
		[ -f ${DATA_DIR}/index.php ] || cat > ${DATA_DIR}/index.php <<< '<? phpinfo(); ?>'
	fi
fi

if [[ "${IPIP}" =~ ^[eE][nN][aA][bB][lL][eE]$ ]]; then
	if ! ls ${NGINX_INSTALL_DIR}/conf | grep -q ip_dabases; then
		mkdir -p ${NGINX_INSTALL_DIR}/conf/ip_dabases
	fi
	if ! ls ${NGINX_INSTALL_DIR}/conf/ip_dabases| grep -q 17monipdb.datx; then
		cp -a ${Nginx_Conf_Dir}/17monipdb.datx ${NGINX_INSTALL_DIR}/conf/ip_dabases/
	fi
	if ! grep -q ipip_db ${NGINX_INSTALL_DIR}/conf/nginx.conf; then
		sed -i "/log_format upstream2/i \    ipip_db ${NGINX_INSTALL_DIR}/conf/ip_dabases/17monipdb.datx 60m; # 60 minute auto reload db file" ${NGINX_INSTALL_DIR}/conf/nginx.conf
	fi
	if ! grep -q ipip_parse_ip ${NGINX_INSTALL_DIR}/conf/nginx.conf; then
...
if [ -n "$REWRITE" ]; then
	[ ! -d ${NGINX_INSTALL_DIR}/conf/rewrite ] && mkdir -p ${NGINX_INSTALL_DIR}/conf/rewrite
	if [ "$REWRITE" = "wordpress" ]; then
	[ ! -f ${NGINX_INSTALL_DIR}/conf/rewrite/wordpress.conf ] && cp ${Nginx_Conf_Dir}/rewrite/wordpress.conf ${NGINX_INSTALL_DIR}/conf/rewrite/wordpress.conf
	elif [ "$REWRITE" = "discuz" ]; then
	[ ! -f ${NGINX_INSTALL_DIR}/conf/rewrite/discuz.conf ] && cp ${Nginx_Conf_Dir}/rewrite/discuz.conf ${NGINX_INSTALL_DIR}/conf/rewrite/discuz.conf
	elif [ "$REWRITE" = "opencart" ]; then
	[ ! -f ${NGINX_INSTALL_DIR}/conf/rewrite/opencart.conf ] && cp ${Nginx_Conf_Dir}/rewrite/opencart.conf ${NGINX_INSTALL_DIR}/conf/rewrite/opencart.conf
	elif [ "$REWRITE" = "laravel" ]; then
	[ ! -f ${NGINX_INSTALL_DIR}/conf/rewrite/opencart.conf ] && cp ${Nginx_Conf_Dir}/rewrite/opencart.conf ${NGINX_INSTALL_DIR}/conf/rewrite/opencart.conf
	elif [ ! -f "$REWRITE" = "typecho" ]; then
	[ ! -f ${NGINX_INSTALL_DIR}/conf/rewrite/typecho.conf ] && cp ${Nginx_Conf_Dir}/rewrite/typecho.conf ${NGINX_INSTALL_DIR}/conf/rewrite/typecho.conf
	elif [ ! -f "$REWRITE" = "ecshop" ]; then
	[ ! -f ${NGINX_INSTALL_DIR}/conf/rewrite/ecshop.conf ] && cp ${Nginx_Conf_Dir}/rewrite/ecshop.conf ${NGINX_INSTALL_DIR}/conf/rewrite/ecshop.conf
	elif [ ! -f "$REWRITE" = "drupal" ]; then
	[ ! -f ${NGINX_INSTALL_DIR}/conf/rewrite/drupal.conf ] && cp ${Nginx_Conf_Dir}/rewrite/drupal.conf ${NGINX_INSTALL_DIR}/conf/rewrite/drupal.conf
	elif [ ! -f "$REWRITE" = "joomla" ]; then
	[ ! -f ${NGINX_INSTALL_DIR}/conf/rewrite/joomla.conf ] && cp ${Nginx_Conf_Dir}/rewrite/joomla.conf ${NGINX_INSTALL_DIR}/conf/rewrite/joomla.conf
	fi
fi
...
python $PYTHON3_INSTALL_DIR/bin/supervisord -n -c /etc/supervisord.conf
